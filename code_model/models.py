...
class GapAwareScheduler():
    ...
    def clip(self, lr, bounds=(1e-6, 0.001)):
        """Clip to bounds"""
        if lr>bounds[1]:
            return bounds[1]
        elif lr<bounds[0]:
...
class CWGAN(pl.LightningModule, ganUtils):
    ...
    def on_train_epoch_end(self):
        """Run at the end of training epoch"""
        # Generate samples
        try:
...
class Diffusion(pl.LightningModule):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        """Run at the end of training step"""
